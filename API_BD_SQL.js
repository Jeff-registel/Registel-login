...
module.exports = () => {
        APP_PACK.app.get("/BD", async (req, res) => {
                ...
                let usuarioPeticion = URLParams.get("usuario-peticion");
                if (usuarioPeticion) {
                        usuarioPeticion = JSON.parse(usuarioPeticion);
                }
                ...
                if (macro) {
                        ...
                        let rutaMACRO_EQUIPO = [root, "SQL", ...partesQuery].join("/") + ".js";
                        let rutaREAD_EQUIPO = [root, "SQL", ...partesQuery].join("/");
                        ...
                        if (!_fs.existe(rutaMACRO_EQUIPO)) { //La macro no se encontró en el equipo
                                return res.json({
                                        error: "No se encontró la macro",
                                });
                        }

                        if (!cabeza) { //No hay parametros, la macro es directa

...
function ALIAS({ estilo, alias, url }) {
        ...
        switch (estilo) {
                case "login":
                        let PK = require("./@ALIAS-LOGIN.json")[alias];
                        return memoria.tools.Array2Nodo(url.replace(cabeza, `${PK}.json`).split("/")).cabeza;
                default:
                        return {};
        }
}
...
function AUTH(query) {
        ...
        if (LOGIN) {
                PK = require(`./@ALIAS-LOGIN.json`)[LOGIN];
        }
        if (PK) {
                usuario = require(`./${PK}.json`);
        }
        if (usuario) {
                ...
                atributos["AUTH"] = AUTH;
                let INTENTOS_FALLIDOS_DE_SEGUIDO = AUTH["INTENTOS_FALLIDOS_DE_SEGUIDO"] ?? [];

                if (INTENTOS_FALLIDOS_DE_SEGUIDO.length >= 3) { //Penalización por intentos fallidos de seguido
                        ...
                        if (DIFERENCIA < segundos * 1000) {
                                return {
                                        ...
                                        error: `Demasiados intentos fallidos de seguido. Intente de nuevo en ${cuentaRegresiva.toFixed(0)} segundos.`,
                                };
                        }
                }
